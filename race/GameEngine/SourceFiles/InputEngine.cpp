...
void InputEngine::axisEventHandler(GLfloat X, GLfloat Y, INPUT_TYPES type)
{
	...
	case INPUT_TYPES::LOOK_AXIS:
	{
		...
		if (this->cameraIndependant)
		{
			...
			Quaternion framePitch;
			Quaternion frameYaw;
			Quaternion q;
			_camera_p->_transform._orientation += framePitch.MakeQFromEulerAngles(Vector3(-Y, 0, 0) * _deltaTime) * _camera_p->_transform._orientation;
			_camera_p->_transform._orientation += frameYaw.MakeQFromEulerAngles(Vector3(0, X, 0) * _deltaTime) * _camera_p->_transform._orientation;
